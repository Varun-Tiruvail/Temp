...
class HierarchyWindow(QMainWindow):
    ...
    #     employee_dict = {emp[0]: {'item': QStandardItem(emp[1]), 'data': emp} for emp in employees}

    #     for emp_id, emp_data in employee_dict.items():
    #         name, position, manager_id = emp_data['data'][1], emp_data['data'][2], emp_data['data'][3]
    #         if manager_id is None:
    #             self.model.appendRow(emp_data['item'])
    #         else:
    #             parent_item = employee_dict[manager_id]['item']
    #             parent_item.appendRow(emp_data['item'])
    #         emp_data['item'].setData(position, role=Qt.ItemDataRole.UserRole + 1)
    ...
    #         name = item.text()
    #         position = item.data(Qt.ItemDataRole.UserRole + 1)
