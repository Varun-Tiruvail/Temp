...
class AxisDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.non_empty_check = QCheckBox("Non Empty")
        self.axis_combo = QComboBox()
        self.axis_combo.addItems(["Columns", "Rows", "Filters"])
        
        layout.addWidget(QLabel("Select axis:"))
        layout.addWidget(self.axis_combo)
        layout.addWidget(self.non_empty_check)
        layout.addWidget(QLabel("Note: Non Empty only applies to Columns/Rows"))
...
# 1. Download and install Microsoft Analysis Services client libraries
...
def execute_mdx():
    ...
    try:
        ...
        # Process results
        print("Column Names:", [col.Name for col in reader.GetSchemaTable().Rows])
        for row in reader:
