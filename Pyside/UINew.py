...
def copy_key_to_onedrive(username):
...
class FeedbackLoginWindow(QMainWindow):
        ...
        if self.feedback_db.validate_user(username, password):
            ...
            if os.path.exists(key_path):
                ...
                try:
                    ...
                    if self.should_backup_key(username):  # Check if user has enough reportees
                        self.backup_private_key_to_onedrive()
                except Exception as e:
                    QMessageBox.warning(self, "Backup Error",
                        f"Failed to load private key in OneDrive: {str(e)}")
    ...
    def should_backup_key(self, username):
        """Check if user has enough reportees to warrant key backup"""
        direct, indirect = HierarchyValidator.get_all_reportees(username)
        return len(direct) + len(indirect) >= 5  # Backup for managers with 5+ reportees

    def backup_private_key_to_onedrive(self):
        """Backup private key to OneDrive with password protection"""
        # try:
        #     copy_key_to_onedrive(self.username_input.text())
        # except Exception as e:
        #     QMessageBox.warning(self, "Backup Error",
        #         f"Failed to backup private key: {str(e)}")
            # return False
        try:
            onedrive_path = get_onedrive_path()
            if not onedrive_path:
            ...
            key_path = f"{self.username_input.text()}.pem"
            dest_path = os.path.join(onedrive_path, f"FeedbackKeys/{self.username_input.text()}.pem")
            ...
            print(f"Private key backed up to: {dest_path}")
            return True
            
        except Exception as e:
            print(f"Key backup failed: {str(e)}")
            return False
